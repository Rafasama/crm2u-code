...
<div class="container-fluid" id="CustomLabelsContainer">
    ...
    <br>
    <label class="pull-left themeTextColor font-x-x-large">{vtranslate('LBL_CUSTOM_LABELS','EMAILMaker')}</label>
    <br clear="all">{vtranslate('LBL_CUSTOM_LABELS_DESC','EMAILMaker')}
    <hr>
    <br />
    <div class="row-fluid">
        <label class="fieldLabel"><strong>{vtranslate('LBL_DEFINE_CUSTOM_LABELS','EMAILMaker')}:</strong></label><br>    
        <div class="row-fluid">           
            <button type="button" class="btn btn-danger span1 marginLeftZero" onclick="confirm_delete();">{vtranslate('LBL_DELETE')}</button>    
            <div class="pull-right btn-group">
                <button type="button" class="addCustomLabel btn addButton marginLeftZero" data-url="?module=EMAILMaker&view=IndexAjax&mode=editCustomLabel"><i class="icon-plus icon-white"></i>&nbsp;<strong> {vtranslate('LBL_ADD')}</strong></button>
        ...
        <div class="pushDownHalfper">    
            ...
            <thead>
                <tr class="blockHeader">
                    ...
                    <th style="border-left: 1px solid #DDD !important;" width="30%">{vtranslate('LBL_KEY','EMAILMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="50%" colspan="2">{vtranslate('LBL_CURR_LANG_VALUE','EMAILMaker')} ({$CURR_LANG.label})</th>
                    <th style="border-left: 1px solid #DDD !important;" width="16%" align="center">{vtranslate('LBL_OTHER_LANG_VALUES','EMAILMaker')}</th>
                </tr>
            </thead>
            <tbody>
            ...
            {foreach key=label_id item=label_value from=$LABELS name=lbl_foreach}
                <tr class="opacity">
                    <td><input type="checkbox" name="chx_{$label_id}" id="chx_{$smarty.foreach.lbl_foreach.index}"/></td>
                    <td><label class="CustomLabelKey textOverflowEllipsis">{$label_value.key}</label></td>
                    <td><label class="CustomLabelValue textOverflowEllipsis">{$label_value.lang_values.$lang_id}</label></td>
                    <td style="border-left: none;">
                        <div class="pull-right actions">
                            <a class="editCustomLabel cursorPointer" data-url="?module=EMAILMaker&view=IndexAjax&mode=editCustomLabel&labelid={$label_id}&langid={$lang_id}">
                                <i title="Edit" class="icon-pencil alignBottom"></i></a>&nbsp;
                        </div>
                    </td>
                    <td><a class="showCustomLabelValues textOverflowEllipsis cursorPointer" data-url="?module=EMAILMaker&view=IndexAjax&mode=showCustomLabelValues&labelid={$label_id}&langid={$lang_id}">{vtranslate('LBL_OTHER_VALS','EMAILMaker')}</a></td>
                </tr>
            {foreachelse}
                <tr id="noItemFountTr"><td colspan="3" class="cellText" align="center" style="padding:10px;"><strong>{vtranslate('LBL_NO_ITEM_FOUND','EMAILMaker')}</strong></td></tr>
            {/foreach}
            </tbody>
        </table>
        </div>
