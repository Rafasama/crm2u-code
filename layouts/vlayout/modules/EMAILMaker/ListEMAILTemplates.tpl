...
<script>
    ...
    {rdelim}
    {rdelim}
    function SaveTemplatesOrder() {ldelim}
        $("vtbusy_info").style.display = "inline";
        var tmpl_order = '';
        for (i = 0; i < document.massdelete.elements.length; i++) {ldelim}
            var elm = document.massdelete.elements[i];
            if (elm.type == 'text' && elm.name.indexOf('tmpl_order_', 0) == 0) {ldelim}
                if ((isNaN(elm.value) == false && elm.Value != '')) {ldelim}
                    var templateid = elm.name.split('_', 3)[2];
                    var order = elm.value;
                    tmpl_order += templateid + '_' + order + '#';
    {rdelim} else {ldelim}
                    alert('{vtranslate("LBL_ORDER_ERROR","EMAILMaker")}');
                    elm.focus();
    ...
    {/literal}
       alert('{vtranslate("LBL_ORDER_SAVE_OK","EMAILMaker")}');
    {literal}
       } else {
    {/literal}
       alert('{vtranslate("LBL_ORDER_SAVE_ERROR","EMAILMaker")}');
    {literal}
                                                     }
                                                     $("vtbusy_info").style.display = "none";
                                                 }
...
<div class="contentsDiv marginLeftZero">
    <div class="listViewPageDiv">
        ...
        {include file='ListEMAILHeader.tpl'|@vtemplate_path:'EMAILMaker'}
        <div class="listViewContentDiv" id="listViewContents">
            {include file='ListEMAILTemplatesContents.tpl'|@vtemplate_path:'EMAILMaker'}
         </div>
        </form>    
    </div>
</div>